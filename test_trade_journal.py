...
j.create_queued_trade(2, 'TSLA', 'long', 10.0, 15.0, 5.0, 'notes', 4, 'metadata')
...
t = trade.Trade(10, 'TSLA', 2, 3, 50, 300, 200, 150, 305, 199.5, 'CLOSED', 4, 5,'long',1)
j.create_trade_record(t, 'notes', 6, 'metadata')
...
assert rows[0][18] == 'Expiration in Days', f'Queued Trade [0][18]: {rows[0][18]}'
assert rows[0][19] == 'Metadata', f'Queued Trade [0][19]: {rows[0][19]}'
...
assert str(rows[1][13]) == '', f'Queued Trade [1][13]: {rows[1][13]}'
...
assert str(rows[1][18]) == '6', f'Queued Trade [1][18]: {rows[1][18]}'
assert str(rows[1][19]) == 'metadata', f'Queued Trade [1][19]: {rows[1][19]}'

t.ticker = 'AAPL'
j.update_trade_record(t)
