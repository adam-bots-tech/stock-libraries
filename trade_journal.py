...
class TradeJournal():
	...
	def __create_trade_journal__(self):
		...
		queued_trades.updateRow(1, ['Ticker', 'Type', 'Entry Price', 'Exit Price', 'Stop Loss', 'Notes', 'Expiration in Days', 'Amount', 'Sell At End of Day'])
		...
		trades.updateRow(1, ['ID', 'Create Date', 'Ticker', 'Type', 'Status', 'Entry Date', 'Exit Date', 'Planned Entry Price', 'Planned Exit Price', 
			'Stop Loss', 'Shares', 'Entry Price', 'Exit Price', 'Gain', 'Buy Order', 'Sell Order', 'Notes', 'Comments', '', 'Buy Metadata','Sale Metadata'])
		prep = ss.createSheet('Preparation')
		prep.updateRow(1, ['Ticker', 'Type', 'Entry Price', 'Exit Price', 'Stop Loss', 'Notes', 'Expiration in Days', 'Amount', 'Sell At End of Day'])
		return ss
	...
	def create_queued_trade(self, row_num, ticker, type, entry, exit, stop_loss, notes, expiration, amount, end_of_day):
		self.journal.refresh()
		self.journal[0].updateRow(row_num, [ticker, type, entry, exit, stop_loss, notes, expiration, amount, end_of_day])
	...
	def create_trade_record(self, trade, notes):
		...
		self.journal[1].updateRow(trade.id + 1, [trade.id, trade.create_date, trade.ticker, trade.type, trade.status, trade.entry_date, trade.exit_date, 
			...
			trade.actual_exit_price, '', trade.buy_order_id, trade.sell_order_id, notes, '', '', '', ''])
	...
	def update_trade_record(self, trade, sale_metadata=None, buy_metadata=None):
		...
		self.journal[1].updateRow(trade.id + 1, [trade.id, trade.create_date, trade.ticker, trade.type, trade.status, trade.entry_date, trade.exit_date, 
			trade.planned_entry_price, trade.planned_exit_price, trade.stop_loss, trade.shares, trade.actual_entry_price, 
			trade.actual_exit_price, gain, trade.buy_order_id, trade.sell_order_id, notes, comments, '', buy_metadata, sale_metadata])
